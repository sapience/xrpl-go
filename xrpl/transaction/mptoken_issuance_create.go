...
const (
  ...
  tfMPTCanLock uint32 = 2
  
  // If set, indicates that individual holders must be authorized.
  // This enables issuers to limit who can hold their assets.
  tfMPTRequireAuth uint32 = 4
  
  // If set, indicates that individual holders can place their balances into an escrow.
  tfMPTCanEscrow uint32 = 8
  
  // If set, indicates that individual holders can trade their balances
  //  using the XRP Ledger DEX or AMM.
  tfMPTCanTrade uint32 = 16

  // If set, indicates that tokens may be transferred to other accounts
  //  that are not the issuer.
  tfMPTCanTransfer uint32 = 32

  // If set, indicates that the issuer may use the Clawback transaction
  // to clawback value from individual holders.
  tfMPTCanClawback uint32 = 64
)

var (
	ErrTransferFeeRequiresCanTransfer = errors.New("MPTokenIssuanceCreate: TransferFee cannot be provided without enabling tfMPTCanTransfer flag")
	ErrInvalidMaximumAmount           = errors.New("MPTokenIssuanceCreate: invalid MaximumAmount, must be a valid unsigned 64-bit integer")
	ErrInvalidMPTokenMetadata         = errors.New("MPTokenIssuanceCreate: MPTokenMetadata must be a valid hex string and at most 1024 bytes")
)
...
type MPTokenIssuanceCreate struct {
   ...
   // ```
	MaximumAmount types.CurrencyAmount `json:",omitempty"`
...
func (m *MPTokenIssuanceCreate) Validate() (bool, error) {
	...
	if m.TransferFee > 0 {
		if m.TransferFee > MAX_TRANSFER_FEE {
			return false, errors.New("MPTokenIssuanceCreate: TransferFee must be between 0 and 50000")
		}
	...
	if !typecheck.IsHex(m.MPTokenMetadata) {
		return false, errors.New("MPTokenIssuanceCreate: MPTokenMetadata must be in hex format")
	}
