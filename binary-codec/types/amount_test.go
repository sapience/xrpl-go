...
func TestAmount_ToJson(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			malleate: func(t *testing.T) interfaces.BinaryParser {
				return serdes.NewBinaryParser([]byte{0x40})
			},
			expected: nil,
		...
		},
		{
			name: "pass - positive native xrp",
			malleate: func(t *testing.T) interfaces.BinaryParser {
				return serdes.NewBinaryParser([]byte{0x40, 0x23, 0x86, 0xf2, 0x6f, 0xc1, 0x00, 0x00})
			},
		...
		},
		{
			name: "pass - positive issued currency",
			malleate: func(t *testing.T) interfaces.BinaryParser {
				return serdes.NewBinaryParser([]byte{0xd8, 0x83, 0x8d, 0x7e, 0xa4, 0xc6, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x55, 0x53, 0x44, 0x0, 0x0, 0x0, 0x0, 0x0, 0x69, 0xd3, 0x3b, 0x18, 0xd5, 0x33, 0x85, 0xf8, 0xa3, 0x18, 0x55, 0x16, 0xc2, 0xed, 0xa5, 0xde, 0xdb, 0x8a, 0xc5, 0xc6})
			},
